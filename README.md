...
Daily weight and caloric data are inherently noisy (water retention, glycogen shifts). Gryzor filters the noise to find the trend:
    ...
    31-Day Horizon: Monthly canvases for Macro and Body Composition trends.

    The Signal Toggle: Dynamically recalculates jagged daily inputs into a 7-Day Trailing Average to reveal the true metabolic trajectory.
...
Advanced metrics derived from interdisciplinary fields (Psychology, Economics, and Physiology):
    ...
    Recovery Debt Ratio: Monitors CNS fatigue by weighting "Grind" sessions against "Rest" tags, with a multiplier for workouts performed under recovery stress.

    Weekly P&L: A financial-style ledger comparing Theoretical Fat Loss (calculated from deficit) against Actual Scale Delta to audit TDEE accuracy.
...
🎙️ High-Fidelity Capture
    ...
    Tactile Feedback: Deep integration with the hardware haptic engine (LongPress voltage) for a satisfying, physical interaction.
...
This application was entirely architected and coded for my personal use through a continuous human-AI pair programming workflow.
